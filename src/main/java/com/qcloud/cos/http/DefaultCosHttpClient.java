    ...
    private void setMultiPartEntity(HttpPost httpPost, Map<String, String> params)
        ...
        for (String paramKey : params.keySet()) {
            if (paramKey.equals(RequestBodyKey.FILE_CONTENT)) {
                StringBody stringBody = new StringBody(params.get(RequestBodyKey.FILE_CONTENT),
                        "application/octet-stream", Charset.forName("ISO-8859-1"));
                multipartEntity.addPart(RequestBodyKey.FILE_CONTENT, stringBody);
            } else {
                StringBody paramValue = new StringBody(params.get(paramKey), "text/plain",
                        Charset.forName("UTF-8"));
                multipartEntity.addPart(paramKey, paramValue);
            }
        }
