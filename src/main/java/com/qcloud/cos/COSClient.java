...
public class COSClient implements COS {
	...
	public String createFolder(CreateFolderRequest request) {
		try {
            // 因数平不知道为何会创建根目录, 当路径是为/时, 直接返回成功
            if (request.getCosPath().equals('/')) {
                JSONObject responseObj = new JSONObject();
                responseObj.put(ResponseBodyKey.CODE, 0);
                responseObj.put(ResponseBodyKey.MESSAGE, "SUCCESS");
	...
	@Override
	public String copyFile(CopyFileRequest request) {
		try {
			return fileOp.copyFile(request);
		} catch (AbstractCosException e) {
			recordException("copyFile", request, e.toString());
			return e.toString();
		} catch (Exception e) {
			UnknownException e1 = new UnknownException(e.toString());
			recordException("copyFile", request, e1.toString());
			return e1.toString();
		}
	}
